...
public class BotAgent1 extends Agent {
	// Constants
	// ////////////////////////////////
					...
					if (msg != null) {
						if (HELLO.equals(msg.getContent())) {
							...
							if (m_guestCount == m_guestList.size()) {
								System.out
										.println("All guests have arrived, starting conversation");
	...
	protected void inviteGuests(int nGuests) {
		...
		m_partyOver = false;
		((BotUIFrame) m_frame).lbl_numIntroductions.setText("0");
		((BotUIFrame) m_frame).prog_rumourCount.setValue(0);
		((BotUIFrame) m_frame).lbl_rumourAvg.setText("0.0");
		...
		try {
			for (int i = 0; i < nGuests; i++) {
				...
				AgentController guest = container.createNewAgent(localName,
						"examples.party.GuestAgent", null);
				guest.start();
	...
	protected void endParty() {
		...
		System.out
				.println("Simulation run complete. NGuests = "
						+ m_guestCount
						+ ", time taken = "
						+ m_avgFormat.format(((double) System
								.currentTimeMillis() - m_startTime) / 1000.0)
						+ "s");
	...
	protected void doIntroduction(AID guest0) {
		if (!m_partyOver) {
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					((BotUIFrame) m_frame).lbl_numIntroductions.setText(Integer
							.toString(m_introductionCount));
				}
			});
			updateRumourAvg();
		}
	...
	protected void incrementRumourCount() {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				((BotUIFrame) m_frame).prog_rumourCount.setValue(Math.round(100
		...
		if (m_rumourCount == m_guestCount) {
			...
			try {
				SwingUtilities.invokeAndWait(new Runnable() {
					public void run() {
						((BotUIFrame) m_frame).btn_stop_actionPerformed(null);
					}
				});
			} catch (Exception e) {
				e.printStackTrace();
	...
	protected void setPartyState(final String state) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				((BotUIFrame) m_frame).lbl_partyState.setText(state);
			}
		});
	}
	...
	protected void updateRumourAvg() {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				((BotUIFrame) m_frame).lbl_rumourAvg.setText(m_avgFormat
						.format(((double) m_introductionCount) / m_rumourCount));
			}
		});
	}
